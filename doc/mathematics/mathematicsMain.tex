...
In the practical CPD applications, the quality of a model is often related to interpretability ofnthe results rather
than mathematical properties, the most useful model is the one that aligns with the "naked eye" or provides the most coherent narrative for decision-making. The usefulness of a function depends on its aesthetic clarity or on its predictive accuracy. In order to align with visual expectation, most selection approaches allow for penalizations that permit to align the results to visual expectations. The resulting functions can be viewed as instruments for identifying useful approximations rather than a {\em true} model, which is virtually never contained in real-world candidate sets [10].

However, user-defined penalizations assume a human feedback that, given the nature of the problem, should be tailored as a per-series input. In order to facilitate the automation of the CPD analysis, we tested a number of alternative cost functions, both coming from information-theoretic and optimization contribution, aiming at a reasonably resilient alternative.
