...
The experiments were conducted on the M3, M4 and M6 benchmark datasets in order to provide baseline results for comparison with the proposed set-covering MILP framework. 
...
The results reported in Tables~\ref{tab:PELTseriesAICQRMSE}--\ref{tab:WBSeriesAICQRMSE} highlight several differences among the three approaches. In general, the number of detected segments varies substantially depending on both the algorithm and the cost function. 

In particular, the AIC criterion generally favors more fragmented segmentations while the QRMSE criterion tends to produce smoother segmentations with fewer changepoints.
In our implementation, QRMSE is computed as a quarter-root RMSE, obtained from the residual sum of squares as

\[
QRMSE = \sqrt{\frac{SSE}{\sqrt{n}}}
\]

which effectively corresponds to scaling the residual sum of squares with an exponent proportional to $n^{1/4}$. 
This formulation reduces the implicit penalty associated with long segments compared with classical RMSE or MSE-based criteria. 
As a consequence, longer segments are penalized less strongly and the resulting segmentations tend to be smoother.

The influence of this formulation is clearly visible in Table~1. When the PELT algorithm is used with the same penalty parameter $\beta = 40$ for both cost functions, the QRMSE criterion generally produces fewer segments than AIC across most dataset categories. 
...
\begin{tabular}{@{}cl|lr|rr|rr|rr|rr}
	...
	Bench & Category & avg.n & max.n & avg.t & max.t & avg.n & max.n & avg.t & max.t & med.t & max.t \\
	\midrule
...
\begin{tabular}{@{}cl|rr|rr|rr|rr|rr}
	\toprule
	& & \multicolumn{4}{c|}{AIC} & \multicolumn{4}{c}{QRMSE} & \multicolumn{2}{|c}{TSPP} \\
	Bench & Category & avg.n & max.n & avg.t & max.t & avg.n & max.n & avg.t & max.t & med.gap & max.gap \\
	\midrule
