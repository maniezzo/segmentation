...
\begin{itemize}
	...
	\item {\it Equal or bounded segment sizes}: Require near-equal segment lengths or bound length variability (e.g., max/min segment length ratio), affecting the global partition shape. (GLOBALE SE NON SO PRIMA LA LUNGHEZZA; LOCALE SE LA SO)
...
\begin{table}[H]
	\centering
	\caption{PELT: Data series results, QRMSE cost}\label{tab:seriesQRMSE}%
...
\begin{table}[H]
	\centering
	\caption{PELT: Data series results, AIC cost}\label{tab:seriesAIC}%
	\begin{tabular}{lllrrrrrrr}
		\toprule
		benchmark & category & cost & avg.n & max.n & avg.nruns & max.nruns & avg.nsegm & max.nsegm & avg.t \\
		\midrule
