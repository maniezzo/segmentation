...
\begin{itemize}
	...
	Bounds on the length of each segment prevent over-fragmentation and excessively long segments. This constraint is expressed as
	\begin{equation}
	...
	Enforce a minimum gap of at least $L$ points between consecutive changepoints. It can be written as
	    \begin{equation}
		e - s + 1 \geq L ,
	\end{equation}
	
	\item Slope constraints.
	When segments are modeled via linear regression, constraints can be imposed on segment-specific parameters, such as the slope $\beta_{s,e}$. Typical examples include bounded slopes, 
	\begin{equation}
	...
	Changepoints may be restricted to a predefined set $\mathcal{A} \subseteq \{1,\dots,N\}$  of admissible positions, such as calendar or seasonal markers. In this case, segment boundaries must satisfy:
	    \begin{equation}
		s,e \in \mathcal{A}.
	\end{equation}
...
\begin{itemize}
	...
	\begin{equation}
		\sum_{j} c_j x_j \le B,
	\end{equation}
	where $c_j$ denotes the cost of segment $j$. 
	...
	\end{equation}
	where $\mu_{t^-}$ and $\mu_{t^+}$ denote the segment parameters before and after a changepoint.
