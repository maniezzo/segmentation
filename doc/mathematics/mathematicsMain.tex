...
We also ran the TSPP model while constraining it to generate exactly the same number of segments as Dynp, in order to allow a fair performance comparison in a setting compatible with the other algorithms. In this case, both TSPP and Dynp yield optimal solutions; therefore, only the computational times are reported for TSPP.

The higher asymptotic complexity and longer setup time of TSPP are evident in Tables \ref{tab:PELTseriesAICQRMSE} and \ref{tab:DYNPseriesAICQRMSE}. In particular, the degradation in TSPP performance becomes apparent for longer time series.
...
Finally, Table \ref{tab:DYNPseriesAICQRMSE} also compares the relative effectiveness of TSPP and WSB. The median percentage error of WSB is generally small, with medians below 3\% and equal to 0\% for the M6 series. However, in some cases the heuristic solutions fail to converge to an acceptable quality.
