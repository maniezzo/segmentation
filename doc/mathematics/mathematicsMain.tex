...
\highlight{
Unlike conventional methods based on dynamic programming (DP), which often impose strict structural constraints on the objective function (e.g., superadditivity), our formulation uses Integer Linear Programming (ILP).  Note that the additivity assumption on the cost function is retained in our model; the flexibility of the proposed framework lies not in relaxing additivity, but in its ability to incorporate both local and global structural constraints directly into the optimization model. The introduction of such constraints, ranging from simple segment-level restrictions to complex global conditions coupling multiple segments, constitutes the main contribution of this work.  This approach offers significantly increased expressiveness and flexibility in the cost structure, accommodating diverse loss functions and complex penalty schemes within an additive cost structure.
}
...
\highlight{Note that no optimality gap is reported for these comparisons: since both TSSP, Pelt and Dynp are exact optimization methods that globally minimize the same additive objective, they are guaranteed to reach the same optimal cost. Therefore, reporting a cost gap would be uninformative; only computational times are compared.}
...
\highlight{It should be noted that the set covering relaxation TSSC, while computationally more efficient than the exact TSSP formulation, yields a heuristic solution: the postprocessing step required to resolve overlapping runs does not preserve the global optimality guarantee. In our experiments, the average optimality gap remained consistently below $1\%$, confirming the practical effectiveness of the heuristic; nonetheless, this distinction should be kept in mind when selecting between the two variants.
