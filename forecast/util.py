...
def read_series(filedata):
   ...
   return name,categ,ds,int(m),int(validation),min,max
...
def reconstruct_AR1(t1, t2, y, m, coeff_seas,nforecast=6):
   ...
   arfit, _ = go_AR1(ysegm, nforecast,m=0)  # keep this if you still need the recursive forecast elsewhere
...
def reconstruct_RF(t1, t2, y, m, coeff_seas):
   ...
   ysub = ysegm
   #rffit, _ = go_RF(ysub, nforecast, m=m)  # keep this if you still need the recursive forecast elsewhere
   
   y = np.asarray(ysub, dtype=float).reshape(-1)
   #lags = 12 if len(y) < 24 else 18
   lags = 12 # hardwired, also in go_rf
...
def plotSol(name, lstVar, dfModel, dfpoints, yBase, yfore, m, model):
   ...
   #m = 1
   coeff_seas = np.zeros(nforecast) # per dati con stagionalità
   
   fig, ax = plt.subplots(figsize=(10, 6))
   ax.plot(dfpoints, marker='.', linewidth=0, color='blue')
   ax.plot(range(len(dfpoints)-nforecast,len(dfpoints)),yBase,linewidth=3,color="green",label="Base")
   ax.plot(range(len(dfpoints)-nforecast,len(dfpoints)),yfore,linewidth=3,color="red",label="Fore")
   for i in lstVar:
      ...
      if (model == "theta"):
         ...
         return
      print(t1, t2, x)
      ax.plot(x, ypred, linewidth=2, color="red", label=f'segm={i}')
