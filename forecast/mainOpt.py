...
def go_opt(name, categ, dfModels, dfpoints, m, nforecast, lstResults):
   ...
   for idModel in range(len(modelNames)):
      ...
      if (idModel == 0):  # ------------------------------ AR1
         lstVarAR1 = lstVar[:]
         yTrain = yorg[:tstart]
         hwfit, yBase = go_AR1(yTrain, nforecast,m=m)
         ...
         yTest = yorg[t0:-nforecast]
         hwfit, yFore = go_AR1(yTest, nforecast,m=m)
         ...
         print(f"AR: full {rmseBaseAR1:.2f} fore {rmseForeAR1:.2f}")
         util.plotSol(name, lstVarAR1, dfModels, dfpoints, yBase, yFore, m, model=model)
      
      if (idModel == 1):  # ------------------------------ HW
         lstVarHW = lstVar[:]
         yTrain = yorg[:tstart]
         hwfit, yBase = go_HW(yTrain, nforecast, m=m)
         #yBase = yBase + coeff[:len(yBase)]
         diff = np.asarray(dfpoints, dtype=float)[-nforecast:] - yBase
         ...
         t0 = lstIntervals[lastInt][0]
         yTest = yorg[t0:-nforecast]
         hwfit, yFore = go_HW(yTest, nforecast, m=m)
         #yFore = yFore + coeff[:len(yFore)]
         diff = np.asarray(dfpoints, dtype=float)[-nforecast:] - yFore
      ...
      if (idModel == 3):  # ------------------------------ RF
         lstVarRF = lstVar[:]
         yTrain = yorg[:tstart]
         hwfit, yBase = go_RF(yTrain, nforecast,m=m)
         #yBase = yBase + coeff[:len(yBase)]
         diff = np.asarray(dfpoints, dtype=float)[-nforecast:] - yBase
         ...
         t0 = lstIntervals[lastInt][0]
         yTest = yorg[t0:-nforecast]
         hwfit, yFore = go_RF(yTest, nforecast,m=m)
         #yFore = yFore + coeff[:len(yFore)]
         diff = np.asarray(dfpoints, dtype=float)[-nforecast:] - yFore
   ...
   print(f"{name}, n.intervals {len(lstVarAR1)} RMSE base ar = {rmseBaseAR1:.2f}, RMSE fore ar = {rmseForeAR1:.2f}")
